...
public class WheelController : MonoBehaviour
{
    ...
    void WheelControl()
    {
        for (int i = 0; i < steerableWheels.Length; i++)
        {
            ...
            steerableWheels[i].steeringAngle =
                Mathf.LerpAngle(steerableWheels[i].steeringAngle, 0,
            ...
            {
                steerableWheels[i].wheelCol.motorTorque =
                    -Mathf.Lerp(steerableWheels[i].wheelCol.motorTorque, wheelMaxSpeed,
                    Time.deltaTime * wheelAcceleration);
            }
            ...
            if (Vertical < -0.1)
            {
                steerableWheels[i].wheelCol.motorTorque =
                    Mathf.Lerp(steerableWheels[i].wheelCol.motorTorque, wheelMaxSpeed,
                    Time.deltaTime * wheelAcceleration * BreakPower);
                RB.drag = 0.3f;
            }
            else
            {
            ...
            {
                steerableWheels[i].steeringAngle =
                    Mathf.LerpAngle(steerableWheels[i].steeringAngle, -wheelSteeringAngle,
                    Time.deltaTime * wheelRotateSpeed);
            }
